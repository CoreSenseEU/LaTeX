...
%\delno{D4.3}
%\background{CSirisbackground.jpg}
%\background{nobackground.png}
...
\versionitem{0.3}{Changes to adapt to base class commands' redefinitions}{R.Sanz}{2023/01/18}
...
\begin{center}
\includegraphics[height=10cm,frame]{csreportcover.png}\includegraphics[height=10cm,frame]{csreportpage.png}
\end{center}
...
\item Images --- a set of PDF/PNG images with the CORESENSE logo, the partners logos, the EU emblems, and the cover background.
\end{itemize}
...
\bko CSUltraLightGreen & HTML(\#EEFFFA) & \color{CSUltraLightGreen}\rule{1cm}{1cm}\color{white}\rule{0cm}{1.07cm}\\\hline
\bkl CSLightOrange & HTML(\#FAD85D) & \color{CSLightOrange}\rule{1cm}{1cm}\color{white}\rule{0cm}{1.07cm}\\\hline
\bko CSUltraLightOrange & HTML(\#FFEECC) & \color{CSUltraLightOrange}\rule{1cm}{1cm}\color{white}\rule{0cm}{1.07cm}\\\hline
...
\code|\textbackslash documentclass\{article\}|
...
\code|\textbackslash usepackage\{fancyhdr\}|
...
These are the macros provided by the template; some of them are the same as the base \code|article| class:
...
\item[\textbackslash orangecover\{\}] --- Select the plain orange cover for deliverables. 
\item[\textbackslash greencover\{\}] --- Select the plain green cover (e.g. for internal reports). 
\item[\textbackslash marooncover\{\}] --- Select the plain maroon cover (e.g. for internal reports). 
...
\item[\textbackslash code||] --- Change font to monospaced colored (to highlight code). Beware this is an \LaTeX\ environment and requieres vertical bars (||) instead of braces (\{\}). 
...
\item Set the plain orange cover instead of the iris background cover with \code|\orangecover|.
\item Use \code|\subtitle{}| to make any precision or highlight some aspect of particular relevance at the time of deliverable release (e.g. if releasing in draft status). You may also also leave it empty.
\item Identify the partner organisation in the authors list (e.g. \code|\author{R.Sanz (UPM)}|).
\item Establish the dissemination level of the deliverable with \code|\dissemination{}|. Our dissemination levels are only two: \code|Public| and \code|Sensitive|.
\item Verify that the list of document versions is complete and correct.
\item Add a \code|\approval{}| section after the list of versions (when the deliverable is approved by WPL, PC, and QA).
\end{enumerate}
...
\begin{center}
\includegraphics[height=8cm,frame]{csdeliverablecover.jpg}
\end{center}

Beware that deliverables shall follow the QA process stated in the PMP \cite{CS-000}. This means they shall be approved by AA partner and the PC. Use the \code|\approval| command for this. Beware this command has six arguments (WPL approver, date; PC approver, date; QA approver, date). For example, use the following code for including the approval for a deliverable produced by FHG to be QA by IMR:

\begin{lstlisting}[language=TeX,caption={\LaTeX\ code for deliverable approval}]
\approval{N. Hammoudeh (FHG)}{2022/12/13}
         {R. Sanz (UPM)}{2022/12/14}
         {C. García (IMR)}{2022/12/14}
\end{lstlisting}

Will render this deliverable approval text into the document at the bottom of the page where it is used:
...
\addcontentsline{toc}{chapter}{Annex: The class file}

This is the \code|csreport.cls| class file used to generate this report.

\lstinputlisting[language=TeX]{csreport.cls}

% Back matter ===================================================================

\bibliography{library,coresense}
